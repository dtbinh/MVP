...
legend('Freestream', 'Rotor 1 - Lead','Rotor 2 - Right','Rotor 3 - Rear','Rotor 4 - Left')
...
hold off
legend('Pitch', 'Rotor 1 - Lead','Rotor 2 - Left','Rotor 3 - Rear','Rotor 4 - Right')
...
title('vi_z,x/V_{\infty} - Normal Induced Velocity in X-Direction Global/Freestream Velocity')
...
title('vi_z,y/V_{\infty} - Normal Induced Velocity in Y-Direction Global/Freestream Velocity')
...
figure;
plot(...flowV, flowV./flowV,'k',...
    flowV,vi_int_total(:,3,1),'--ro',...
    flowV,vi_int_total(:,3,2),'--b+',...
    flowV,vi_int_total(:,3,3),'--ks',...
    flowV,vi_int_total(:,3,4),'--gx')
title('z-dir')

figure;
plot(...flowV, flowV./flowV,'k',...
    flowV,vi_int_total(:,1,1),'-.ro',...
    flowV,vi_int_total(:,1,2),'-.b+',...
    flowV,vi_int_total(:,1,3),'-.ks',...
    flowV,vi_int_total(:,1,4),'-.gx')
title('x-dir')

figure;
plot(...flowV, flowV./flowV,'k',...
    flowV,vi_int_total(:,2,1),'-.ro',...
    flowV,vi_int_total(:,2,2),'-.b+',...
    flowV,vi_int_total(:,2,3),'-.ks',...
    flowV,vi_int_total(:,2,4),'-.gx')
title('y-dir')
...
%% compare inflow angles with velocity
...
hold on
 %figure;
plot(...flowV, pitchVEHICLEdeg./pitchVEHICLEdeg,'k',...
    flowV,angRATIO_int_only(:,1),'-.ro',...
...
figure;
plot(...flowV, flowV,'k',...
    flowV,magvi_int_only(:,1)-flowV,'-.ro',...
    flowV,magvi_int_only(:,2)-flowV,'-.b+',...
    flowV,magvi_int_only(:,3)-flowV,'-.ks',...
    flowV,magvi_int_only(:,4)-flowV,'-.gx')

figure;
plot(...flowV, flowV-flowV,'k',...
    flowV,(rotorVelINFLOW(:,1)-flowV)./flowV,'-ro',...
    flowV,(rotorVelINFLOW(:,2)-flowV)./flowV,'-b+',...
    flowV,(rotorVelINFLOW(:,3)-flowV)./flowV,'-ks',...
    flowV,(rotorVelINFLOW(:,4)-flowV)./flowV,'-gx')

%% angle compare with and without fuselage interference

pitch = [pitchVEHICLEdeg zeros(length(pitchVEHICLEdeg),1) zeros(length(pitchVEHICLEdeg),1)];
% alpha_vi = 
% FREEplusINT = pitch + alpha_vi;
VR_INT = sqrt(sum(abs(FREEplusINT).^2,2))
% FREEplusINTplusBODY = FREEplusINT + vi_body;
VR_INT_BODY = sqrt(sum(abs(FREEplusINTplusBODY).^2,2))
