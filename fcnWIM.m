function [vi_int,vi_self,skewRAD,wi] = fcnWIM(geomDIAMETER,geomNumBLADES,rotorHUBLOCATIONS,rotorTHRUST,rotorAngINFLOW,rotorVelINFLOW,flowRHO,rotorRPM,geomNumROTORS)
% This is the vectorize version of WIN_V12.m
...
u_wake = (rotorVelINFLOW.*cosd(rotorAngINFLOW));
% Velocity perpendicular to plane plus velocity induced at rotor disc 6.15 in mccormick and find in leishmann
w_wake = 0.5*(rotorVelINFLOW.*sind(rotorAngINFLOW)+sqrt((rotorVelINFLOW.*sind(rotorAngINFLOW).^2+2.*rotorTHRUST/(flowRHO*rotorAREA))));
skewRAD=atan2(u_wake,w_wake); %calculate skew angle of wakes for influencing rotor 3.18 issi thesis

vecZOFFSET = 1/(rotorRPM./60).*0.5.*(rotorVelINFLOW.*sind(rotorAngINFLOW)+sqrt((rotorVelINFLOW.*sind(rotorAngINFLOW)).^2+(2.*rotorTHRUST./(flowRHO.*rotorAREA))))*(1/geomNumBLADES);
